...
以及其他普通共享模式音频都会被包含。若功能探测失败，程序会枚举所有活动输出设备，
分别对可用设备回环并以 10 ms 有界时钟混音；只有在没有可用输出设备时才切换到“默认输出最终回退”。
界面会显示当前模式和活动输出数量。不同设备有独立时钟，因此混音器优先保证有界延迟，
并记录丢帧/欠载；长期漂移和镜像输出去重仍需目标硬件实测。
...
已完成：Flutter 静态分析与单元测试、Windows 原生严格编译、全局/多输出混音/默认输出
三个分支的本地假接收端集成测试、Android 单元测试/检查/构建及权限清单检查。
